<!--Empresas-->
<div id='wrapper_title'>
    <div class='tagpost-top section' id='tagpost-top'>
        <div class='widget HTML' id='HTML5'>
            <div>
                <h3 class="main_title">Todas as ligações - <u>Empresas</u></h3>
            </div>
        </div>
    </div>
</div>


<div class="card-deck text-center mx-auto w-75">
    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>

    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg">
            </div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h1 class="card-title sub_title">FNAC</h1>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>

    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>
</div>

<div class="card-deck text-center mx-auto w-75">
    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>

    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>

    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>
</div>

<div class="card-deck text-center mx-auto w-75">
    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>

    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg">
            </div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h1 class="card-title sub_title">FNAC</h1>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>

    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>
</div>

<div class="card-deck text-center mx-auto w-75">
    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>

    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>

    <div class="cards">
        <div class="card-item">
            <div class="card-image">
                <img class="imagem" src="img/fnac.jpg"></div>
            <div class="card-info">
                <h4 class="card-intro description_title">Trabalhar</h4>
                <h2 class="card-title sub_title">FNAC</h2>
                <p class="card-intro description_title">Ver vagas disponíveis</p>
            </div>
        </div>
    </div>
</div>
